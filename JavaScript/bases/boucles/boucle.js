"use strict"
// ----------------------------- WHILE --------------------
//* While est l'équivalent de Tant que
...
//La boucle attends un booléen true ou false.
//Ici notre boucle étant à true, elle ne s'arrêtera jamais
while(true){
    a++;
    if(a<17){
        //le mot clé continue stoppe la répétition en cours et passe à la suivante.
        //Ici le console.log n'apparaitra qu'à parttir de 17.
        continue;
    }
    console.log("a vaut ",a);
    
    if (a==20){
        //L'utilisation du mot clé beak permet aussi la sortie d'une boucle.
        break;
    }
}
...
for(let i=0; i<10; i++){
    ...
    Le troisième de combien doit augmenter (ou diminuer) la variable
    à chaque itération
    */
...
for(let food in arr){
    ...
    Si le tableau contient 3 éléments, le for in fera 3 itérations, sa 
    variable valant à tour de role 0, 1 puis 2
    */
    console.log(food, " -> ", arr[food]);
}
for (let carac in person) {
    //utilisé sur un objet, on obtient le nom de toutes les propriétés.
    console.log(carac, " -> ", person[carac]);
}

//syntaxe: for("déclration de variable" in "tableau/objet")
for(let f of arr) {
    /*
for of fonctionne comme for in mais au lieu de récupérer à chaque itération l'index,, il récupère
...
A chacune des itérations l'élément suivant du tableau est rangé dans la variable
...
//todo On poussera un peu plus loin forEach lorsque l'on verra les fonctions.
